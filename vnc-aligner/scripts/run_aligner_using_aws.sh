...
S3_MIP_TEMPLATES_MOUNTPOINT=${S3_MIP_TEMPLATES_MOUNTPOINT:-"/s3_mip_templates"}
S3_VNC_TEMPLATES_MOUNTPOINT=${S3_VNC_TEMPLATES_MOUNTPOINT:-"/s3_vnc_alignment_templates"}
...
while [[ $# > 0 ]]; do
    ...
    case $key in
        ...
        --outputs-s3bucket-name)
            ...
            ;;
        --mip-templatedir)
            mip_templates_dir_param="$1"
            shift # past value
            ;;
        --vnc-templatedir)
            vnc_templates_dir_param="$1"
            shift # past value
            ;;
        --search-id)
            searchId="$1"
